...
layout: post
excerpt: 스프링 부트 (Spring boot) 기반으로 JPA, Lombok을 사용하여 H2 데이타베이스에 데이타를 쓰고 읽은 값을 리턴하는 REST API 웹서비스를 구현해 본다.
comments: yes
toc: true
---

`스프링 부트 (Spring boot) 기반으로 JPA, Lombok을 사용하여 H2 데이타베이스에 데이타를 쓰고 읽은 값을 리턴하는 REST API 웹서비스를 구현해 본다.`

## I. 스프링 부트 프로젝트 생성

스프링 부트 프로젝트를 생성하는것은 이전 글 [스프링 부트 (Spring boot) 소개]({% post_url 2019-01-25-kang.heehun@introduce-spring-boot %}) 를 참고하면 된다.  
본 프로젝트 생성시 "Lombok", "Web", "JPA", "H2" 4가지 모듈을 넣어서 생성하면 된다.
