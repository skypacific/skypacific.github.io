---
published: true
title: 스프링 부트 (Spring boot) JPA, Lombok, H2 를 이용 REST API 구현
categories: Spring
...
`스프링 부트 (Spring boot) 기반으로 JPA를 사용하여 H2 데이타베이스에 데이타를 쓰고 읽은 뒤 REST API를 제공하는 코드를 작성해 본다.`

## I. 스프링 부트 프로젝트 생성

스프링 부트 프로젝트를 생성하는것은 이전 글 [스프링 부트 (Spring boot) 소개]({% post_url 2019-01-25-kang.heehun@introduce-spring-boot %}) 를 참고하면 된다.  
프로젝트 생성시 "Lombok", "Web", "JPA", "H2" 4가지 모듈을 넣어서 생성하면 된다.

각 스프링 부트 스타터 모듈들의 기능은 다음과 같다.

### 1. Lombok
* 자바에서 모델(=엔티티) 클래스 작성시에 Lombok을 사용하면 Constructor, Getter/Setter, toString 등 컴파일시에 자동으로 생성해준다.
* Constructor 의 경우, 파라메터가 없는 경우와 파라메터를 모두 받는 경우 2가지를 생성해준다.
* @Log 어노테이션을 사용하면 log 필드가 자동생성이된다.
...
* Web 모듈을 사용하면 REST API (RESTful 웹서비스) 구현을 쉽게 할 수 있다.
...
* ApplicationRunner 인터페이스를 생성해주는 applicationRunner 메소드를 만든다.
...
* 위의 ApplicationRunner 에서 저장했던 3개의 Product 들이 JSON 결과로 리턴된다.
  ...
  ![Product 클래스 생성](/assets/images/sbjpa01_rest_products.png)
...
* 위의 ApplicationRunner 에서 저장했던 id가 2인 Product 가 JSON 결과로 리턴된다.
  ...
  ![Product 클래스 생성](/assets/images/sbjpa01_rest_product2.png)
