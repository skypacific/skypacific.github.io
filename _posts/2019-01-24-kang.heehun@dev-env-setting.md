...
tags: [Developer, Enviroment, 개발자, 개발환경]
layout: post
...
Windows에서는 기본적으로 패키지 관리자가 없기 때문에 별도로 패캐지 관리자를 설치해야 한다.

여기서는 가장 많이 사용하는 Chocolatey 를 사용한다.
...
여기서는 Linux OS 중에서 대표적으로 많이 사용되는 Ubuntu 10.x를 기반으로 진행한다.

기본적으로 패키지 관리자 apt 는 설치되어 있으므로 apt 설치과정은 생략한다.
