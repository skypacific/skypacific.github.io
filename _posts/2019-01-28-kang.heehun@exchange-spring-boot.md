...
<i class="nf nf-dev-java"></i> 자바 어플리케이션이 Exchange 서버에 엑세스 방법은 EAS 와 EWS 2가지 방법이 있다.
...
* <i class="nf nf-mdi-file_xml"></i> 프로젝트 루트에 있는 Maven 디펜던시 파일 (pom.xml)을 열고 아래 디펜던시를 추가한다.
    ```xml
    <dependency>
...
### 1. <i class="nf nf-oct-terminal"></i> 터미널(컴맨드) 쉘에서 JAR 파일 빌드하고 실행
...
<i class="nf nf-fa-instagram"></i> [sbtest02.zip 소스파일]({{ "/assets/download/sbtest02.zip" | absolute_url }})
